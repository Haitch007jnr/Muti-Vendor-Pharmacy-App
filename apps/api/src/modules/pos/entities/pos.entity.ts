...
export class PosSession {
  ...
  @Column({ type: 'decimal', precision: 15, scale: 2, name: 'opening_balance', default: 0 })
  openingBalance: number;

  @ApiProperty({ description: 'Closing balance', required: false })
  @Column({ type: 'decimal', precision: 15, scale: 2, name: 'closing_balance', nullable: true })
  closingBalance: number;

  @ApiProperty({ description: 'Expected balance', required: false })
  @Column({ type: 'decimal', precision: 15, scale: 2, name: 'expected_balance', nullable: true })
  expectedBalance: number;

  @ApiProperty({ description: 'Cash sales' })
  @Column({ type: 'decimal', precision: 15, scale: 2, name: 'cash_sales', default: 0 })
  cashSales: number;

  @ApiProperty({ description: 'Card sales' })
  @Column({ type: 'decimal', precision: 15, scale: 2, name: 'card_sales', default: 0 })
  cardSales: number;

  @ApiProperty({ description: 'Bank transfer sales' })
  @Column({ type: 'decimal', precision: 15, scale: 2, name: 'bank_transfer_sales', default: 0 })
  bankTransferSales: number;

  @ApiProperty({ description: 'Total sales' })
  @Column({ type: 'decimal', precision: 15, scale: 2, name: 'total_sales', default: 0 })
  totalSales: number;
...
export class PosTransaction {
  ...
  @Column({ type: 'decimal', precision: 15, scale: 2, name: 'change_amount', default: 0 })
  changeAmount: number;
  ...
  @ManyToOne(() => PosSession, (session) => session.transactions, { onDelete: 'CASCADE' })
