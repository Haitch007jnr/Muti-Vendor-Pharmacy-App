...
export class Inventory {
  ...
  @ApiProperty({ description: 'Reorder level - alerts when stock reaches this level' })
  @Column({ type: 'int', name: 'reorder_level', default: 10 })
  reorderLevel: number;

  @ApiProperty({ description: 'Reorder quantity - suggested quantity to reorder' })
...
export class InventoryAdjustment {
  ...
  @ApiProperty({ description: 'Adjustment status', enum: InventoryAdjustmentStatus })
  @Column({ type: 'varchar', length: 50, default: InventoryAdjustmentStatus.APPROVED })
